...
注意 `label()` 中的 `match` 覆盖了所有四个变体，没有使用通配符 `case _`。这意味着如果将来新增一个 `Reconnecting` 状态，编译器会在每一处未处理该变体的 `match` 表达式上报错——遗漏的状态在编译期就被捕获，而非在运行时悄无声息地失败。对于 AIChatPro 这样的长连接应用，漏掉一个连接状态可能导致界面冻结或消息丢失，穷举匹配正是防止此类问题的第一道防线。
...
上面的 `Sat | Sun => "周末"` 将共享相同逻辑的两个变体合并到一条分支中，避免了重复代码。OR 模式在变体数量较多时尤其有用——比如将多个同义别名映射到同一操作，一行即可表达意图。

---

简单枚举为状态赋予名称，但现实中的状态往往携带上下文——频率限制需要告知重试等待时间，网络错误需要附带描述信息。带载荷的枚举正是为此而生：它让每个变体不仅有身份，还有内容。
...
`match` 在匹配带载荷枚举时会自动**解构**——每个 `case` 将载荷中的数据绑定到局部变量（如 `msg`、`secs`、`code`），使其可以直接在分支体中使用。这种方式比手动类型转换或条件判断安全得多：编译器保证你只能在正确的分支中访问对应的数据，类型完全匹配，无需任何强制转换。
...
将 `isRetryable()` 和 `retryAfter()` 定义为枚举方法，让错误本身承载了重试策略——调用方无需关心错误的具体类型，只需询问"能否重试"和"等多久"。AIChatPro 在 API 调用失败时正是利用这种模式实现自动重试：错误对象自己决定恢复方式，调用逻辑保持简洁。

---

除了基本的枚举匹配，仓颉的模式匹配还支持元组、嵌套结构和守卫条件——它更像是一种面向数据结构的微型查询语言，让复杂的分支逻辑变得声明式且易读。
...
元组模式可以理解为**多维分支**：同时对模型名称和可用状态进行匹配，一步到位地覆盖所有组合，比嵌套的 `if-else` 结构更扁平、更清晰。当 AIChatPro 需要根据"模型 × 可用性"矩阵决定行为时，元组模式是最自然的表达方式。
...
注意 `Available(name, Pro(quota))` 在一行中完成了两层解构——从 `ModelInfo` 取出模型名称，再从嵌套的 `ModelTier` 取出配额数值。这种深层模式匹配让 AIChatPro 可以优雅地表达分层模型访问权限：免费用户、Pro 用户各自走不同的路径，无需额外的类型检查或中间变量。

---

AI API 调用中"找不到"是常态——模型别名可能拼错、配置键可能缺失、JSON 字段可能不存在。`Option<T>` 将"值可能缺失"提升为类型系统的一等公民，让编译器强制你处理每一个"空"的可能性，而不是将其留作运行时的隐患。
...
`??` 运算符的语义是"提供一个兜底值"——当 `Option` 为 `Some` 时取出内部值，为 `None` 时使用右侧的默认值。相比 `if-else` 手动判空，`??` 将意图压缩到一行，代码更简洁也更不易出错。
...
`if-let` 是一种**条件解包**：只有当 `Option` 为 `Some` 时才绑定内部值并进入 `if` 分支，否则走 `else` 分支处理缺失情况。与异常处理不同，`Option` 用于**预期中的缺失**（如别名未注册），而异常用于**意料之外的故障**（如网络中断）。两者各司其职，不应混用。
...
`??` 和 `if-let` 是处理 `Option` 最常用的两种方式，在第 8 章（错误处理）中，我们将看到它们如何与异常机制协作——`Option` 处理"可预期的缺失"，`try-catch` 处理"不可预期的失败"，二者共同构成仓颉的完整错误处理体系。

---

至此，我们已经掌握了枚举的所有核心形态：简单枚举定义状态名称、载荷枚举携带上下文数据、`Option` 处理值的缺失，以及模式匹配对它们的穷举解构。接下来的命令系统将把这些概念融为一体——`Role` 使用简单枚举定义角色身份，`CommandResult` 使用载荷枚举表达解析结果，`parseCommand` 则借助 `match` 和 `Option` 完成从用户输入到结构化命令的转换。
...
回顾上面的完整流程：用户输入一行文本 → `parseCommand` 将其解析为 `CommandResult` 枚举值（借助 `??` 处理未知别名）→ `executeCommand` 对枚举穷举匹配并生成响应字符串。整条管线类型安全、分支清晰、无遗漏——这正是枚举与模式匹配在实际项目中的典型协作方式。
