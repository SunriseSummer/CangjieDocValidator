...
本文档介绍测试框架的设计方案与使用方法，包括文档中的标注约定、工作机制和命令行用法。
...
1. **运行前必须配置仓颉 SDK 环境**：确保 `cjpm` 命令可用
2. **Python 安装 tree-sitter-cangjie 插件**：https://github.com/SunriseSummer/CangjieTreeSitter/releases
3. **代码不做修改**：从文档提取的代码原样写入项目，不做任何自动修改（`package` 声明除外），以确保测试能真实反映代码质量
4. **默认保留测试项目**：生成的 `cjpm` 项目默认不删除，方便调试。使用 `--clean` 可在测试后清理
5. **通用设计**：本工具不绑定特定目录结构，可用于检查任何包含仓颉 Markdown 文档的目录
6. **并发输出不确定性**：包含并发代码（`spawn`）的示例，输出顺序可能不固定，此类示例建议不设置 `expected_output`
7. **未标注警告**：未标注的代码块不会导致测试失败（退出码仍为 0），但会在报告中输出警告，便于开发者持续整改
